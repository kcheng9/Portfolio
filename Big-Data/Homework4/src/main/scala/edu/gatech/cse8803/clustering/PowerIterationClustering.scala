...
object PowerIterationClustering {
  ...
  def runPIC(similarities: RDD[(Long, Long, Double)]): RDD[(Long, Int)] = {
    ...
    val picSim = new PIC()
    picSim.setK(3)
    val picRun = picSim.run(similarities)

    val assignments = picRun.assignments

    assignments.cache()

    val result = assignments.map(m => (m.id, m.cluster)).collect
    ...
    val clusteringResult = sc.parallelize(result)
